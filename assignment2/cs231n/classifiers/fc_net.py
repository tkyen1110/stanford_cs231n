...
class TwoLayerNet(object):
    ...
    def __init__(self, input_dim=3*32*32, hidden_dim=100, num_classes=10,
        ...
        # H = hidden_dim
        # C = num_classes
        # self.params['W1'] = np.random.normal(loc=0.0, scale=weight_scale, size=(D,H))
        # self.params['b1'] = np.zeros(H)
        # self.params['W2'] = np.random.normal(loc=0.0, scale=weight_scale, size=(H,C))
...
class FullyConnectedNet(object):
    ...
    def loss(self, X, y=None):
        ...
        for i in range(self.num_layers-1):
            if self.normalization == 'batchnorm':
                layer_input, ar_cache[i] = affine_bn_relu_forward(layer_input,
                                           self.params['W%d'%(i+1)], self.params['b%d'%(i+1)],
                                           self.params['gamma%d'%(i+1)], self.params['beta%d'%(i+1)], self.bn_params[i])
            elif self.normalization == 'layernorm':
                layer_input, ar_cache[i] = affine_ln_relu_forward(layer_input,
                                           self.params['W%d'%(i+1)], self.params['b%d'%(i+1)],
                                           self.params['gamma%d'%(i+1)], self.params['beta%d'%(i+1)], self.bn_params[i])
            else:
        ...
        out, ar_cache[self.num_layers-1] = affine_forward(layer_input, self.params['W%d'%(self.num_layers)], self.params['b%d'%(self.num_layers)])
        scores = out
        # *****END OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
        ############################################################################
        #                             END OF YOUR CODE                             #
        ...
        for i in range(self.num_layers-1, 0, -1):
            ...
            if self.use_dropout:
                dout = dropout_backward(dout, dp_cache[i-1])

            if self.normalization == 'batchnorm':
                dx, dw, db, dgamma, dbeta = affine_bn_relu_backward(dout, ar_cache[i-1])
            elif self.normalization == 'layernorm':
                dx, dw, db, dgamma, dbeta = affine_ln_relu_backward(dout, ar_cache[i-1])
            else:
                dx, dw, db = affine_relu_backward(dout, ar_cache[i-1])
